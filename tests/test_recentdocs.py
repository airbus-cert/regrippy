...
@pytest.fixture
def mock_reg():
    key = RegistryKeyMock.build("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\RecentDocs")
    ...
    document = RegistryValueMock("a", b"m\x00y\x00d\x00o\x00c\x00u\x00m\x00e\x00n\x00t\x00.\x00d\x00o\x00c\x00x\x00\x00random_bytes\xCA\xFE", RegBin)
...
def test_recentdocs(mock_reg):
    ...
    assert(results[0].custom["docname"] == "mydocument.docx"), "The document should be named 'mydocument.docx'"
