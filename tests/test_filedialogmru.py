...
@pytest.fixture
def mock_reg():
    key = RegistryKeyMock.build("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32")
...
def test_filedialogmru(mock_reg):
    ...
    assert(results[0].value_data in ["evil.bat", "%USERPROFILE%\\Desktop"]), "The first result should be either from OpenSave or LastVisited"
    
    if results[0].value_data == "evil.bat":
        # OpenSaveMRU
        assert(results[1].value_data == "LOVE-LETTER-FOR-YOU.TXT.vbs"), "The second result should be 'a'"
        assert(results[2].value_data == "quarterly_reports.docx"), "The third result should be 'r'"

        # Then LastVisitedMRU
        assert(results[3].value_data == "%USERPROFILE%\\Desktop"), "The fourth result shoul be 'a'"
        ...
        assert(results[3].value_data == "LOVE-LETTER-FOR-YOU.TXT.vbs"), "The fourth result should be 'a'"
        assert(results[4].value_data == "quarterly_reports.docx"), "The fifth result should be 'r'"