...
from Registry.Registry import RegSZ, RegDWord
import pytest
...
@pytest.fixture
def mock_reg():
    key = RegistryKeyMock.build(r"Software\Microsoft\Windows\CurrentVersion\Internet Settings")
    ...
    # Add a Proxy.pac
    pac = RegistryValueMock("AutoConfigURL", "http://proxy.internal.corp/proxy.pac", RegSZ)
...
def test_proxy(mock_reg):
    ...
    assert proxy.custom["proxy"] == "172.16.2.34:8080", "Proxy should be set to '172.16.2.34:8080'"
    assert proxy.custom["exceptions"] == "10.0.0.0/8", "There should be a single exception"

    assert pac.custom["proxypac"] == "http://proxy.internal.corp/proxy.pac", "The ProxyPAC should be properly configured"