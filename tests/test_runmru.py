...
@pytest.fixture
def mock_reg():
    key = RegistryKeyMock.build("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\RunMRU")
...
def test_runmru(mock_reg):
    ...
    assert(results[0].value_data == "first result"), "The first result should be 'first result'"

    assert(results[1].value_name == "c"), "The second result key should be 'c'"
    assert(results[1].value_data == "second result"), "The second result should be 'second result'"

    assert(results[2].value_name == "a"), "The third result key should be 'a'"
    assert(results[2].value_data == "third result"), "The third result should be 'third result'"
