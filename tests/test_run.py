...
def mock_software():
    ...
    policies_run = reg.open("Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer\\Run")
...
@pytest.fixture
def mock_ntuser():
    curver_run = RegistryKeyMock.build("Software\\Microsoft\\Windows\\CurrentVersion\\Run")
    ...
    winnt = RegistryKeyMock.build("Windows NT\\CurrentVersion\\Windows\\Run").root().open("Windows NT")
    reg.open("Software\\Microsoft").add_child(winnt)

    winnt_run = reg.open("Software\\Microsoft\\Windows NT\\CurrentVersion\\Windows\\Run")
