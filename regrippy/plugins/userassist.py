...
class Plugin(BasePlugin):
    """Parses the UserAssist key to get information on program usage"""
    __REGHIVE__ = "NTUSER.DAT"

    def run(self):
        key = self.open_key("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\UserAssist")
        if not key:
            return
    ...
    def display_machine(self, result):
        ua = result.custom['userassist']
        print(mactime(name=f"{self.guess_username()} {ua.name};{ua.number_of_execs};{ua.focus_count} {ua.focus_time_secs}", mtime=result.mtime, atime=ua.last_exec))
...
class UAObject(object):
    ...
    def __init__(self, name, raw):
        self._raw = raw
        self.name = codecs.encode(name, "rot13")
        for gid, tr in DIR_TRANSLATION.items():
            self.name = re.sub(re.escape(gid), tr, self.name, flags=re.IGNORECASE)

        self.number_of_execs = self._read_nb_execs()
        self.focus_count = self._read_focus_count()
    ...
    def _read_focus_count(self):
        if len(self._raw) == UAObject.LENGTH_WINXP:
            return 0
        
        return int.from_bytes(self._raw[8:12], byteorder="little")
    ...
    def _read_last_exec(self):
        timestamp = 0
        if len(self._raw) == UAObject.LENGTH_WINXP:
            timestamp = int.from_bytes(self._raw[8:16], byteorder="little", signed=False)
        else:
            timestamp = int.from_bytes(self._raw[60:68], byteorder="little", signed=False)
