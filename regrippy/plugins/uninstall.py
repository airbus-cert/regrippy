...
class Plugin(BasePlugin):
    ...
    def display_human(self, result):
        print(f"{result.key_name} ({result.custom['display_name']})\n\t{result.custom['uninstall_string']}\n")
        
    def display_machine(self, result):
        print(mactime(name=f"{result.key_name};{result.custom['display_name']};{result.custom['uninstall_string']}", mtime=result.mtime))
